...
// ── Nav structure ─────────────────────────────────────────────────────────────
// Sections: Overview, Audit Trail, HITL → Integrity Monitor, Anchoring → Policy, Agents, Reports
    ...
    items: [
      {
        ...
        icon: ShieldCheck,
        aliases: ["/console/monitoring/integrity", "/dashboard/monitoring/integrity"],
      },
      {
        href: "/console/anchoring",
        label: "Anchoring",
        icon: Anchor,
        aliases: ["/dashboard/system/anchoring"],
      },
    ],
  },
  {
...
// Mobile tab bar: top 5 → Overview, Audit Trail, HITL, Integrity Monitor, Anchoring
const TAB_BAR_ITEMS = ALL_ITEMS.slice(0, 5);
// More drawer: Policy, Agents, Reports
...
function SidebarNavItem({
  ...
  return (
    <Link
      href={item.href}
      id={`governance-nav-${item.label.toLowerCase().replace(/\s+/g, "-")}`}
...
export default function GovernanceSidebarNav() {
    ...
    <>
      ...
      <aside
        ...
        style={{
          ...
          background:
            "radial-gradient(circle at 20% 8%, rgba(255,196,118,0.048), transparent 50%), linear-gradient(180deg, rgba(255,255,255,0.022), rgba(255,255,255,0.008))",
          scrollbarWidth: "thin",
      ...
      <nav
        id="governance-mobile-tabs"
        aria-label="Governance mobile navigation"
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] font-semibold leading-none tracking-wide">
                {/* Abbreviate "Integrity" → "Integrity", "Anchoring" → "Anchor" etc. */}
                {item.label.split(" ")[0]}
              </span>
            </Link>
          );
      ...
      {drawerOpen && (
        <div className="md:hidden">
          ...
          {/* Drawer panel */}
          <div
            id="governance-more-drawer"
            role="dialog"
            aria-label="More governance sections"
          ...
          >
            ...
            <div className="mb-2 flex items-center justify-between px-2 py-1">
              ...
              <button
                ...
                onClick={() => setDrawerOpen(false)}
                className="flex h-7 w-7 items-center justify-center rounded-full text-white/52 transition-colors hover:text-white"
